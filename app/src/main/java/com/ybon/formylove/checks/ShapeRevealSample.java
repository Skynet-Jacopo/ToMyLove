...
//		paint.setTypeface(robotoBlack);
		...
		Text textA = TextBuilder.create("Now why you loer en kyk gelyk?").setSize(60).setPosition(Align.SURFACE_CENTER).build();
		Text textB = TextBuilder.create("Is ek miskien van goud gemake?").setSize(60).setPosition(Align.BOTTOM_OF | Align.CENTER_OF, textA).build();
		Text textC = TextBuilder.create("You want to fight, you come tonight.").setSize(60).setPosition(Align.BOTTOM_OF | Align.CENTER_OF, textB).build();
		Text textD = TextBuilder.create("Ek moer jou sleg! So jy hardloop weg.").setSize(60).setPosition(Align.BOTTOM_OF | Align.CENTER_OF, textC).build();
